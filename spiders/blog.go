...
func getSign(uuid string, url string) string {
	...
	hmac_str := hmac_encoder.Sum(nil)
	hmac_digest := make([]byte, hex.EncodedLen(len(hmac_str)))
	hex.Encode(hmac_digest, hmac_str)

	fmt.Println(hmac_digest)
...
func main() {
	...
	uuid := "442584db-e5c5-4d3b-6528-3d3c19f2ffa3"
