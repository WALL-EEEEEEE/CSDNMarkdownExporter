...
func getSign(uuid string, url string) string {
	...
	fmt.Println("url_str: ", url_str)
	to_enc := []byte(url_str)
	fmt.Println("to_enc: ", to_enc)
	hmac_encoder := hmac.New(sha256.New, ekey)
	hmac_encoder.Write(to_enc)
	hmac_digest := hmac_encoder.Sum(nil)
	fmt.Println("hmac_hex_digest: ", hex.EncodeToString(hmac_digest))
	fmt.Println("hmac_digest: ", hmac_digest)
...
func main() {
	...
	uuid := createUuid() //"442584db-e5c5-4d3b-6528-3d3c19f2ffa3"
	url := "https://bizapi.csdn.net/blog-console-api/v3/editor/getArticle?id=80341748&model_type="
	...
	http_proxy := func(_req *http.Request) (*Url.URL, error) {
		return Url.Parse("http://192.168.10.4:8888")
	}
